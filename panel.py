...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    add_user(chat_id)
    # Single compact menu grid; no separate/duplicate menu button is shown.
    keyboard = [
            ...
            for pk, pc in panels.items():
                ...
                if not api_url:
                    ...
                    try:
                        ...
                        if dev_node is not None:
                            online_devices, error = await get_online_devices(client, api_url, auth_key, dev_node)
                            if error:
                                status = f"🔴 {error}"
                            else:
                                # Keep Status identical to Monitor Devices: count only online devices.
                                device_count = len(online_devices)
                                total_devices += device_count
                                status = "🟢 Active" if device_count > 0 else "🟡 No Online Devices"
                        else:
...
async def handle_text_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("awaiting_url"):
        ...
        # Accept one link per line, comma-separated links, or several URLs in one message.
        links = re.findall(r'https?://[^\s,]+', text)
        links = [link.rstrip('.,;)]}') for link in links]
        if not links:
            await update.message.reply_text("❌ Koi valid Firebase link nahi mila.")
            return
        panels = load_panels()
        added = 0
        errors = []
        async with httpx.AsyncClient() as client:
            for link in links:
                ...
                if not api_url:
                    errors.append(f"Invalid Firebase link: {link[:80]}")
                    continue
                # discovery
                dev_node, msg_node = await discover_structure(client, api_url, auth_key)
                if dev_node is None:
                    errors.append(f"Structure/access nahi mila: {link[:80]}")
                    continue
                if any(pc.get("api_url") == api_url and pc.get("auth_key", "") == auth_key for pc in panels.values()):
                    errors.append(f"Already added: {link[:80]}")
                    continue
        ...
        result = f"✅ {added} panel(s) add ho gaye!"
        if errors:
            result += "\n\n⚠️ Skipped:\n" + "\n".join(errors[:15])
